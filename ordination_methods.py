...
labelcolors = {e: cmap(float(idx)/N_labels) for idx, e in enumerate(labels)}
datapointcolors = df['label'].map(labelcolors).tolist()
...
plt.title('PCA')
ax1.scatter(PCA_x_scores, PCA_y_scores, c=datapointcolors)
plt.xlabel('X scores')
plt.ylabel('Y scores')
patches = [matplotlib.patches.Patch(color=cmap(idx/N_labels), label=e) for idx, e in enumerate(labels)]
plt.legend(handles=patches)
# MDS
ax2 = fig.add_subplot(1,4,2)
plt.title('MDS')
ax2.scatter(MDS_x_scores, MDS_y_scores, c=datapointcolors)
plt.xlabel('X scores')
plt.ylabel('Y scores')
# NMDS
ax3 = fig.add_subplot(1,4,3)
plt.title('NMDS')
ax3.scatter(NMDS_x_scores, NMDS_y_scores, c=datapointcolors)
plt.xlabel('X scores')
plt.ylabel('Y scores')
# tSNE
ax4 = fig.add_subplot(1,4,4)
plt.title('tSNE')
ax4.scatter(tSNE_x_scores, tSNE_y_scores, c=datapointcolors)
plt.xlabel('X scores')
plt.ylabel('Y scores')
# save
